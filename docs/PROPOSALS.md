...
## A two-stage split, so a melodic stem gets the reference tier
**What it does for a producer:** The reference tier only splits vocals from instrumental. To get a melodic stem — the trumpet, the Rhodes, the thing a producer actually wants to isolate — you must use a `strong`-tier four-stem model, whose own vocal branch is weaker than BS-Roformer's by its author's published numbers (9.0 against 12.98 SDR). That is exactly the case that produced the muddy layers in the first real sessions. A chain would run the reference separator first, then split its instrumental into drums, bass and other, so the melodic stem has had its vocals taken out by the best separator available rather than by the four-stem model's weaker branch.
**Principle it serves:** 6's neighbour — separation is the irreversible step, so the best available should run. Also 4: a clearer melodic stem is a more editable one.
**Principle it risks:** **2, measure don't guess.** There is a real counter-mechanism: two passes are two sets of artefacts, and the second model was trained on full mixes, not on instrumentals, so it runs out of distribution. The chain could be worse. Nothing here should ship on the argument above alone.
**What it takes:** The registry cannot express a chain today — `MODELS` maps one id to one set of outputs, and `resolve_model` returns one `ModelChoice`. It would need a plan of stages with a carry stem, a composed quality record (the tier of a chain is its weakest link; there is no published SDR for a chain and the basis must say so), storage paths and a `stems` label for a two-model run, and a fourth entry in `STEM_SPLITS` that is really a chain.
**What would decide it:** `scripts/quality_chain.py --real` on a GPU, comparing `other` from one-stage `htdemucs_ft` against `other` from `bs_roformer → htdemucs_ft` on the same source: 8–20 kHz energy against the source, transient preservation, and a listen. `quality/chain.py` already measures per stage; it needs a second arrangement to compare against, not new measurements.
**Where it belongs:** after the owner has a GPU and has answered OPEN_QUESTIONS 41 (which checkpoints the image carries) — the chain is moot if the reference tier is not installed.
**Status:** proposed, deliberately unbuilt. Building the capability before the measurement would put an option in front of producers that we cannot say is better, which is the thing this document exists to prevent.
